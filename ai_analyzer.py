...
def generate_report(data: dict) -> dict:
    ...
    # Текущая дата — модель обучена на устаревших данных,
    # без этого она может путать контекст и писать общие фразы
    current_date_ru = datetime.now().strftime("%d.%m.%Y")
    ...
    # Новости — теперь с кратким содержанием, не только заголовком.
    # Раньше передавались только title, из-за чего GPT писал общие
    # фразы без конкретики — не хватало контекста для деталей.
    news_text = ""
    for n in news:
        if not n.get("title"):
            continue
        news_text += f"- {n['title']}"
        if n.get("summary"):
            news_text += f": {n['summary']}"
        if n.get("date"):
            news_text += f" ({n['date']})"
        news_text += "\n"
    news_text = news_text or "Нет данных"
...
3. Конкретика вместо общих слов — каждый пункт bull_case и bear_case должен
   содержать конкретную цифру, дату, название события/продукта/конкурента
   или прямую отсылку к переданным новостям. Общие фразы вроде "может
   увеличить маржу" или "растущая конкуренция" без конкретики ЗАПРЕЩЕНЫ.
4. Сегодняшняя дата: {current_date_ru}. Строй рассуждения и прогнозы исходя
   из того, что сейчас именно эта дата. Твои базовые знания о компании могут
   быть устаревшими — опирайся ИСКЛЮЧИТЕЛЬНО на данные и новости, переданные
   ниже в этом сообщении, а не на то, что ты "помнишь" о компании из обучения.
5. Только валидный JSON без markdown
...
Последние новости (используй конкретные факты из них в bull_case/bear_case/what_is_happening):
{news_text}
...
Формат ответа:
{{
  "what_is_happening": "3-4 абзаца живым языком. Не 'выручка выросла на X%', а 'компания последние полгода делала ставку на AI и это начинает окупаться'. Расскажи историю компании прямо сейчас, опираясь на конкретные новости выше.",
  ...
  "bull_case": [
    "Аргумент написанный как мысль аналитика, не как пункт списка, с конкретным фактом. Например: 'Сервисный сегмент растёт быстрее железа — это меняет всю модель оценки компании'",
    "Аргумент 2 живым языком с конкретикой",
    "Аргумент 3 живым языком с конкретикой",
    "Аргумент 4 живым языком с конкретикой",
    "Аргумент 5 живым языком с конкретикой"
  ],
  "bear_case": [
    "Риск 1 живым языком с конкретикой",
    "Риск 2 живым языком с конкретикой",
    "Риск 3 живым языком с конкретикой",
    "Риск 4 живым языком с конкретикой",
    "Риск 5 живым языком с конкретикой"
  ],
  "swot": {{
  ...
  "interest_reason": "Одно предложение почему именно такой вывод"
    ...
    print(f"Дата в промпте: {current_date_ru}")
